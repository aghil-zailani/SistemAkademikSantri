...
<div class="bg-white rounded-t-lg shadow-sm border border-gray-200 px-6 py-4 flex justify-between items-center">
    <h2 class="text-sm font-semibold text-[#0056b3]">Referensi <span class="text-gray-500">> Mata Pelajaran</span></h2>
            ...
            <tr class="hover:bg-gray-50 transition">
                ...
                <td class="px-6 py-3 text-center">
                    <div class="flex justify-center gap-1">
                        <button onclick="openEditModal({{ $item->id }}, '{{ $item->nama_pelajaran }}')" class="bg-[#0056b3] text-white px-2 py-0.5 text-[11px] rounded hover:bg-blue-700">Ubah</button>
                        <form action="{{ route('mata-pelajaran.destroy', $item->id) }}" method="POST" class="inline" onsubmit="return confirm('Hapus data ini?');">
                            @csrf @method('DELETE')
                            <button type="submit" class="bg-[#f59e0b] text-white px-2 py-0.5 text-[11px] rounded hover:bg-yellow-600">Hapus</button>
                        </form>
                    </div>
                </td>
                <td class="px-6 py-3">{{ $item->nama_pelajaran }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
...
<div id="formModal" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white w-full max-w-md mx-4 rounded-lg shadow-xl p-6">
        <h3 id="modalTitle" class="font-semibold text-gray-800 mb-4">Tambah Mata Pelajaran</h3>
        <form id="mapelForm" method="POST">
            @csrf
            <input type="hidden" name="_method" id="formMethod" value="POST">
            <div class="mb-5">
                <label class="block text-xs text-gray-700 mb-1">Nama Mata Pelajaran</label>
                <input type="text" name="nama_pelajaran" id="inputNama" required class="w-full border-gray-300 rounded border p-2 text-sm focus:ring-blue-500">
            </div>
...
<script>
    ...
    function openEditModal(id, nama) {
        document.getElementById('modalTitle').innerText = 'Ubah Mata Pelajaran';
        document.getElementById('mapelForm').action = `/mata-pelajaran/${id}`;
