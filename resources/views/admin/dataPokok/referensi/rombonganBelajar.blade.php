    ...
    <table class="w-full text-sm text-left">
        <thead class="bg-gray-50 border-b border-gray-200 text-gray-500 uppercase text-[11px]">
            <tr>
                ...
                <th class="px-6 py-3">Nama Rombel</th>
                <th class="px-6 py-3">Wali Kelas</th>
                <th class="px-6 py-3">Kelas (Relasi)</th> </tr>
        </thead>
            ...
            <tr class="hover:bg-gray-50 transition">
                ...
                <td class="px-6 py-3 text-center">
                    <div class="flex justify-center gap-1">
                        <button onclick="openEditModal({{ $item->id }}, '{{ $item->nama }}', '{{ $item->kelas_id }}', '{{ $item->wali_kelas_id }}')" class="bg-[#0056b3] text-white px-2 py-0.5 text-[11px] rounded hover:bg-blue-700">Ubah</button>
                        <button onclick="openEditModal({{ $item->id }}, '{{ $item->nama }}', '{{ $item->kelas_id }}', '{{ $item->wali_kelas_id }}')" class="bg-[#28a745] text-white px-2 py-0.5 text-[11px] rounded hover:bg-green-600">Wali Kelas</button>
                ...
                <td class="px-6 py-3">{{ $item->dataKelas->nama_kelas ?? '-' }}</td> 
            </tr>
            @endforeach
        </tbody>
    </table>
...
<div id="formModal" class="fixed inset-0 z-50 hidden bg-gray-900 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white w-full max-w-md mx-4 rounded-lg shadow-xl p-6">
        ...
        <form id="rombelForm" method="POST">
            ...
            <div class="mb-3">
                <label class="block text-xs text-gray-700 mb-1">Kelas</label>
                <select name="kelas_id" id="inputKelas" required class="w-full border-gray-300 rounded border p-2 text-sm focus:ring-blue-500 bg-white">
...
<script>
    ...
    function openEditModal(id, nama, kelas_id, wali_id) {
        ...
        document.getElementById('inputKelas').value = kelas_id; // Mengisi value select kelas
