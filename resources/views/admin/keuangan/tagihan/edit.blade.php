...
<div class="bg-white rounded-lg shadow-sm border border-gray-200 mb-6">
    <div class="px-6 py-4 border-b border-gray-200">
        <h2 class="text-lg font-semibold text-gray-700">Edit Tagihan</h2>
    </div>
    ...
    <form action="{{ route('tagihan.update', $tagihan->id) }}" method="POST">
        ...
        <div class="px-6 py-5 space-y-4 text-sm">
            ...
            <div>
                ...
                <input type="text" name="nama" id="nama" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 @error('nama') border-red-500 @enderror" value="{{ old('nama', $tagihan->nama) }}" required>
                @error('nama')
            ...
            <div>
                ...
                <select name="akun" id="akun" class="w-full border border-gray-300 rounded-md px-3 py-2 text-gray-700 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 @error('akun') border-red-500 @enderror" required>
                    ...
                    @foreach($akunOptions as $akun)
                        <option value="{{ $akun }}" {{ old('akun', $tagihan->akun) == $akun ? 'selected' : '' }}>{{ $akun }}</option>
                    @endforeach
                </select>
                @error('akun')
                    <span class="text-xs text-red-500 mt-1">{{ $message }}</span>
            ...
            <div>
                ...
                <input type="date" name="tanggal_jatuh_tempo" id="tanggal_jatuh_tempo" class="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 @error('tanggal_jatuh_tempo') border-red-500 @enderror" value="{{ old('tanggal_jatuh_tempo', date('Y-m-d', strtotime($tagihan->tanggal_jatuh_tempo))) }}" required>
