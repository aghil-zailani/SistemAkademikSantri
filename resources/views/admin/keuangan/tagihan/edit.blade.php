...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
    <div class="border-b border-gray-200 pb-4 mb-6">
        <h2 class="text-lg font-semibold text-gray-800">Ubah Tagihan</h2>
    </div>
    ...
    <form action="{{ route('admin.tagihan.update', $tagihan->id) }}" method="POST">
        ...
        <div class="space-y-5">
            <div>
                ...
                <input type="text" name="nama" value="{{ old('nama', $tagihan->nama) }}" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
            ...
            <div>
                ...
                <select name="akun_id" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
                    ...
                    @foreach($akuns as $akun)
                        <option value="{{ $akun->id }}" {{ old('akun_id', $tagihan->akun_id) == $akun->id ? 'selected' : '' }}>
                            {{ $akun->kode }} - {{ $akun->nama }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="tanggal_jatuh_tempo" value="{{ old('tanggal_jatuh_tempo', $tagihan->tanggal_jatuh_tempo) }}" class="w-full border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500" required>
